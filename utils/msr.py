...
"""
@author: yinglei
Modified to accept JSON input and output directory via command line.
"""
...
class Wulff:
        ...
        for m in range(self.face_num):
            ...
            def func(TTT):
                ...
                for k in range(self.nGas):
                    TT[k] = TTT[k]
                # calculate w
                cal_w = np.asmatrix(self.w[m]) * np.asmatrix(TT).T
    ...
    def geometry(self):
        ...
        # Use output directory for all outputs
        os.makedirs(self.output_dir, exist_ok=True)
        
        filename_xyz = os.path.join(self.output_dir, f"{self.ele}_{self.structure}_T_{self.T}_P_{self.P}_cluster.xyz")
        with open(filename_xyz, 'w') as fp_xyz:
            ini_xyz_path = os.path.join(self.output_dir, "ini.xyz")
            with open(ini_xyz_path, 'w') as kmc_ini:
                fp_xyz.write('%d\n' % (N_atom))
        ...
        with open(faceinfo_path, 'w') as fo:
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Generate Wulff cluster structure from JSON input.')
    parser.add_argument('--json', '-j', required=True, help='Path to input JSON file')
    parser.add_argument('--output', '-o', default='.', help='Output directory for XYZ and faceinfo.txt (default: current directory)')
    ...
    # Run Wulff construction
    wulff = Wulff()
    wulff.output_dir = args.output  # set output directory
    success, msg = wulff.get_para(input_data)
    if not success:
        print(f"Error reading parameters: {msg}")
        exit(1)
    
    wulff.gen_coverage()
    ret, msg = wulff.geometry()
    if ret == 0:
        print(f"Geometry construction failed: {msg}")
        exit(1)
    
    print(f"Successfully generated cluster with {wulff.nAtoms} atoms.")
